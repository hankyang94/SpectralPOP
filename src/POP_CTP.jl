function CTP_POP_on_Ball(x,f,g,h,k,R;EigAlg="Arpack",method="LMBM",tol=1e-5,showNormGrad=false)
    @time begin
    l_g=length(g)
    l_h=length(h)
    n=length(x)
    ...
    for j in 1:l_g
        ...
        invb,sol = SpectralPOP.CTP_POP([x;x_slack[1]],-g[j],[h;g[1]-x_slack[1]^2],k,R;method=method,EigAlg=EigAlg,tol=tol,showNormGrad=showNormGrad)
        R_bar-=invb
        println("--------------------------")
    end
    ...
    opt_val,opt_sol = SpectralPOP.CTP_POP([x;x_slack],f,bar_h,k,R_bar;method=method,tol=tol,EigAlg=EigAlg)
    if opt_sol!=[]
        opt_sol=opt_sol[1:n]
    end
    end
...
function CTP_POP_UniqueBallIneq(x,f,h,k,R;method="LMBM",EigAlg="Arpack",tol=1e-5,showNormGrad=false)
    @time begin
    @polyvar x_slack
    opt_val,opt_sol = SpectralPOP.CTP_POP([x;x_slack],f,[h;R-sum(x.^2)-x_slack^2],k,R;method=method,EigAlg=EigAlg,tol=tol,showNormGrad=showNormGrad)
    if opt_sol!=[]
...
function CTP_POP(x::Vector{PolyVar{true}},f::Polynomial{true,Float64},h::Vector{Polynomial{true,Float64}},k::Int64,R::Float64;method="LMBM",EigAlg="Arpack",tol=1e-5,showEvaluation=false,showNormGrad=false)
    @time begin
        if showEvaluation
...
function SpectralSDP(s::Int64,m::Int64,a0::Vector{Float64},a::SparseMatrixCSC{Float64},Ib::Vector{UInt64},Vb::Vector{Float64},invInde::SparseMatrixCSC{UInt64,Int64},CT::Float64;method="LMBM",EigAlg="Arpack",tol=1e-5,showNormGrad=false,showEvaluation=false)
    ...
    if method=="LMBM"
        ...
        function phi(nvar::Cint,xp::Ptr{Cdouble},gp::Ptr{Cdouble})
            ...
            @fastmath grad[:]=CT*LinearOper(s,a,eigvec,showEvaluation=showEvaluation)
            grad[Ib]+=Vb
            if showNormGrad
                global norm_grad=[norm_grad;norm(grad)]
            end
            return(convert(Cdouble,CT*eigval+(zvar[Ib]'*Vb)[1]))
        end
    ...
    elseif method=="PB"
        ...
        function evaluate_f(z::Vector{Float64})
            ...
            subgrads[Ib]+=Vb
            return CT*eigval+(z[Ib]'*Vb)[1], subgrads
        end
    ...
    elseif method=="SketchyCGAL"
        ...
        cons = Vector{Float32}([CT;CT])
        b = zeros(Float64,m)

        b[Ib]=Vb

        R = UInt16(minimum([10;ceil(s/5)])) # rank/sketch size parameter
